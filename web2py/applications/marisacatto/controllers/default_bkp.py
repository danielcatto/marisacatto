...
def index():
    ...
    data_regress = data_regressiva(5)
    ...
    return dict(counter=session.counter, carrinho=session.carrinho, a=a, res=res, resultado=resultado, data_regress=data_regress, form=form)
...
def vender():
    ...
    if form.process().accepted:
        ...
        session.carrinho.append(produtos)
    else:
        produtos = 'vazio'
    print(type(produtos))
    return dict(form=form, produtos=produtos)
